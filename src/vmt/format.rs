...
pub trait VMTData 
    ...
    fn load<R>(source: &mut R) -> Result<Self, VMTLoadError> where R: Read;
}
...
impl FromStr for ShaderType {
    type Err = VMTLoadError;

    fn from_str(s: &str) -> Result<ShaderType, VMTLoadError> {
        match s {
            "LightmappedGeneric"    => Ok(ShaderType::LightmappedGeneric),
            "Lightmapped4WayBlend"  => Ok(ShaderType::Lightmapped4WayBlend),
            ...
            _ => Ok(ShaderType::Unknown(s.to_string()))
        }
    }
}
...
impl FromStr for ParamType {
    ...
    fn from_str(s: &str) -> Result<ParamType, VMTLoadError> {
        match s {
